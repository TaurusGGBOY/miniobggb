...
RC Table::make_record(int value_num, const Value *values, char *&record_out) {
  ...
  if (value_num + table_meta_.sys_field_num() + 1 != table_meta_.field_num()) {
    // printf("%d!=%d\n", value_num + table_meta_.sys_field_num(),
  ...
  for (int i = 0; i < value_num; i++) {
    const FieldMeta *field =
        table_meta_.field(i + 1 + normal_field_start_index);
  ...
  for (int i = 0; i < value_num; i++) {
    ...
    if (value.type == NULLS) {
      ...
      if (field->type() == DATES) {
        ...
        if (date_int < 0) {
          LOG_ERROR("Wrong date format Invalid date type. field name=%s, "
                    "type=%d, but given=%d",
                    field->name(), field->type(), value.type);
          return RC::SCHEMA_FIELD_TYPE_MISMATCH;
        } else {
        ...
        for (int i = 0; i < 2; ++i) {
          ...
          int count = 0;
          if ((ret = data_buffer_pool_->get_page_count(file_id_, &count)) !=
              RC::SUCCESS) {
          ...
          for (int p = 1; p < count; ++p) {
            if ((ret = data_buffer_pool_->get_this_page(
                     file_id_, p, &page_handle)) != RC::SUCCESS) {
              LOG_ERROR("Failed to allocate page when inserting text data. "
                        "file_it:%d, ret:%d",
                        file_id_, ret);
              return ret;
            }
            ...
            if (page_header_->record_capacity == 1 &&
              ...
              LOG_DEBUG("get a empty text data page[%d] for insert text data",
                        p);
              break;
            }
            LOG_DEBUG("get a page[%d] for insert text data,but it's capacity "
                      "is %d,num is %d",
                      p, page_header_->record_capacity,
          ...
          if (!page_handle.open) {
            ...
            if ((ret = data_buffer_pool_->allocate_page(
                     file_id_, &page_handle)) != RC::SUCCESS) {
              LOG_ERROR("Failed to allocate page when inserting text data. "
                        "file_it:%d, ret:%d",
                        file_id_, ret);
              return ret;
          ...
          memcpy(page_handle.frame->page.data +
                     page_header_->first_record_offset,
                 (char *)value.data + i * 2048, 2048);
          memcpy(text_buf + i * 4, &current_page_num, 4);
          data_buffer_pool_->mark_dirty(&page_handle);
          if ((ret = data_buffer_pool_->unpin_page(&page_handle)) !=
              RC::SUCCESS) {
            LOG_ERROR("Failed to unpin page when inserting text data. "
                      "file_it:%d, ret:%d",
                      file_id_, ret);
            return ret;
          }
          // memcpy(&p1,&current_page_num,4);
          // LOG_DEBUG("text data is inserted into
          // page:[%d]",*(int*)(text_buf+4*i));
        }
        LOG_DEBUG("text data is inserted into page:[%d:%d]", *(int *)text_buf,
                  *(int *)(text_buf + 4));
...
class IndexInserter {
public:
  explicit IndexInserter(Index *index, Table *table)
...
static RC insert_unique_index_record_reader_adapter(Record *record,
                                                    void *context) {
...
RC Table::create_index(Trx *trx, const char *index_name,
  ...
  if (is_unique) {
    ...
    rc = scan_record(trx, nullptr, -1, &index_inserter,
                     insert_unique_index_record_reader_adapter);
  } else {
...
RC Table::create_index_by_list(Trx *trx, const char *index_name,
  ...
  if (table_meta_.index(index_name) != nullptr ||
      table_meta_.find_index_by_list(name_vec) != nullptr) {
    return RC::SCHEMA_INDEX_EXIST;
  }
...
class RecordUpdater {
  //仅支持单字段更新
public:
  RecordUpdater(Table &tab, Trx *trx, const Value *value)
      : table_(tab), trx_(trx), value_(value) {}
    ...
    if (field_->type() != value_->type) {
      ...
      if (value_->type == NULLS) {
        ...
        if (field_->type() == DATES && value_->type) {
          ...
          LOG_DEBUG("field type is text page num, value type is char, field "
                    "offset is %d",
                    field_->offset());
          return RC::SUCCESS;
        } else {
  ...
  RC update_record(Record *rec) {
    ...
    if (value_->type == NULLS) {
      ...
      if (field_->type() == DATES) {
        ...
        if ((rc = table_.get_buffer_pool()->get_this_page(
                 table_.file_id_, p1, &ph1)) != RC::SUCCESS) {
          LOG_ERROR("failed to get page[%d:%d] for update text data",
                    table_.file_id_, p1);
          return rc;
        }
        if ((rc = table_.get_buffer_pool()->get_this_page(
                 table_.file_id_, p2, &ph2)) != RC::SUCCESS) {
          LOG_ERROR("failed to get page[%d:%d] for update text data",
                    table_.file_id_, p2);
          return rc;
        }
        ...
        memset(ph1.frame->page.data + page_header_1->first_record_offset, 0,
               2048);
        memset(ph2.frame->page.data + page_header_2->first_record_offset, 0,
               2048);
        memcpy(ph1.frame->page.data + page_header_1->first_record_offset,
               (char *)value_->data, 2048);
        memcpy(ph2.frame->page.data + page_header_2->first_record_offset,
               (char *)value_->data + 2048, 2048);
        table_.get_buffer_pool()->mark_dirty(&ph1);
        table_.get_buffer_pool()->mark_dirty(&ph2);
        LOG_DEBUG("update page[%d:%d] data for update text data,their record "
                  "num is [%d:%d]",
                  p1, p2, page_header_1->record_num, page_header_2->record_num);
        if ((rc = table_.get_buffer_pool()->unpin_page(&ph1)) != RC::SUCCESS) {
          LOG_ERROR("failed to unpin page[%d:%d] for update text data",
                    table_.file_id_, p1);
          return rc;
        }
...
Index *
Table::find_index_multi_index(std::vector<std::string> filter_attr_list) const {
  for (Index *index : indexes_) {
...
IndexScanner *Table::find_index_for_scan(const ConditionFilter *filter) {
  ...
  if (default_condition_filter != nullptr) {
    if (!default_condition_filter->left().is_attr &&
        default_condition_filter->left().value == nullptr) {
      return nullptr;
    }
    if (!default_condition_filter->right().is_attr &&
        default_condition_filter->right().value == nullptr) {
      return nullptr;
    }
  ...
  if (composite_condition_filter != nullptr) {
    ...
    for (int i = 0; i < filter_num; i++) {
      ...
      if (temp->left().is_attr) {
        const FieldMeta *field_meta =
            table_meta_.find_field_by_offset(temp->left().attr_offset);
        if (nullptr == field_meta) {
          LOG_PANIC("Cannot find field by offset %d. table=%s",
        ...
        const FieldMeta *field_meta =
            table_meta_.find_field_by_offset(temp->right().attr_offset);
        if (nullptr == field_meta) {
          LOG_PANIC("Cannot find field by offset %d. table=%s",
                    temp->left().attr_offset, name());
          return nullptr;
...
int Table::null_offset() { return table_meta_.field("null_field")->offset(); }