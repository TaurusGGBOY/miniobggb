...
RC BplusTreeHandler::create_by_list(const char *file_name, std::vector<AttrType> attr_vec, std::vector<int> len_vec, std::vector<int> offsets)
{
  BPPageHandle page_handle;
  IndexNode *root;
  char *pdata;
  ...
  for(int i = 0; i < attr_vec.size(); i++){
    file_header->origin_offsets.push_back(offsets[i]);
    file_header->types.push_back(attr_vec[i]);
    file_header->lens.push_back(len_vec[i]);
    file_header->new_offsets.push_back(sum);
    sum += len_vec[i];
  }
  file_header->attr_length = sum;
  file_header->key_length = sum + sizeof(RID);
  file_header->node_num = 1;
...
int CompareKeyList(const char *pdata, const char *pkey, std::vector<AttrType> types, std::vector<int> lens) { // 简化
    int tmp = 0;
    int offset = 0;
    for (int i = 0; i < types.size(); i++) {
        tmp = CompareKey(pdata + offset, pkey + offset, types[i], lens[i]);
        offset += lens[i];
        if (tmp != 0) {
            break;
        }
...
int CmpKeyList(const char *pdata, const char *pkey, std::vector<int> offsets, std::vector<AttrType> types, std::vector<int> lens, int attr_length)
{
  int result = CompareKeyList(pdata, pkey, types, lens);
  if (0 != result) {
    return result;
  }
  RID *rid1 = (RID *) (pdata + attr_length);
...
RC BplusTreeHandler::find_leaf_multi_index(const char *pkey,PageNum *leaf_page, std::vector<int> offsets, std::vector<AttrType> types, std::vector<int> lens) {
  RC rc;
  BPPageHandle page_handle;
  IndexNode *node;
  char *pdata;
...
RC BplusTreeHandler::insert_into_leaf_multi_index(PageNum leaf_page, const char *pkey, const RID *rid)
{
  int i,insert_pos,tmp;
  BPPageHandle  page_handle;
  char *pdata;
  ...
  for(insert_pos = 0; insert_pos < node->key_num; insert_pos++){
    tmp=CmpKeyList(node->keys + insert_pos * file_header_.key_length, pkey, file_header_.new_offsets, file_header_.types, file_header_.lens, file_header_.attr_length);
    if (tmp == 0) {
      return RC::RECORD_DUPLICATE_KEY;
    }
    if(tmp < 0)
...
RC BplusTreeHandler::insert_into_leaf_after_split_multi_index(PageNum leaf_page, const char *pkey, const RID *rid) {
  RC rc;
  BPPageHandle  page_handle1,page_handle2;
  IndexNode *leaf,*new_node;
  PageNum new_page,parent_page;
  ...
  for(insert_pos=0;insert_pos<leaf->key_num;insert_pos++){
    tmp=CmpKeyList(leaf->keys+insert_pos*file_header_.key_length, pkey, file_header_.new_offsets, file_header_.types, file_header_.lens, file_header_.attr_length);
    if(tmp<0)
      break;
  }
  for(i=0,j=0;i<leaf->key_num;i++,j++){
...
RC BplusTreeHandler::delete_entry_from_node_multi_index(PageNum node_page,const char *pkey, std::vector<int> offsets, std::vector<AttrType> types, std::vector<int> lens) {
  ...
  for(delete_index=0;delete_index<node->key_num;delete_index++){
    tmp = CmpKeyList(node->keys + delete_index * file_header_.key_length, pkey, offsets, types, lens, file_header_.attr_length);
    if(tmp==0)
      break;
  }
  if(delete_index>=node->key_num){
...
RC BplusTreeHandler::delete_entry_internal(PageNum page_num,const char *pkey) {
  ...
  if(file_header_.lens.size()>1){
    rc=delete_entry_from_node_multi_index(page_num, pkey, file_header_.new_offsets, file_header_.types, file_header_.lens);
...
RC BplusTreeHandler::delete_entry_multi_index(const char *record, const RID *rid, std::vector<int> offsets, std::vector<AttrType> types, std::vector<int> lens) {
  ...
  rc=delete_entry_internal(leaf_page,key);
  if(rc!=SUCCESS){
    free(key);
    free(pkey);
    return rc;
...
RC BplusTreeHandler::insert_entry_multi_index(const char *record, const RID *rid, std::vector<int> offsets, std::vector<AttrType> types, std::vector<int> lens){
  LOG_TRACE("enter");
  RC rc;
  PageNum leaf_page;
  BPPageHandle page_handle;
  ...
  if(leaf->key_num<file_header_.order-1){
    ...
    rc=insert_into_leaf_after_split_multi_index(leaf_page,key,rid);
    free(key);
    return SUCCESS;
  }
}
...
RC BplusTreeScanner::open_multi_index(std::vector<std::string> value_list, std::vector<CompOp> comop_list) {
  ...
  value_ = value_copy; // free value_
  rc = index_handler_.find_first_index_satisfied_multi_index(comop_list, value_, &next_page_num_, &index_in_node_, index_handler_.file_header_.new_offsets,index_handler_.file_header_.types, index_handler_.file_header_.lens);
  if(rc != SUCCESS){
    if(rc == RC::RECORD_EOF){
      next_page_num_ = -1;
      index_in_node_ = -1;
  ...
  for( ; next_index_of_page_handle_ < pinned_page_count_; next_index_of_page_handle_++){
    ...
    for( ; index_in_node_ < node->key_num; index_in_node_++){
      if(condition_num <= 0){
          ...
          if (satisfy_conditions(node->keys + index_in_node_* index_handler_.file_header_.key_length)) {
              memcpy(rid, node->rids + index_in_node_, sizeof(RID));
              index_in_node_++;
              return SUCCESS;
          }
...
bool BplusTreeScanner::satisfy_conditions(const char *pkey) {
  for(int i = 0;i<condition_num;i++){
    if(!satisfy_one_condition(pkey+offsets[i], index_handler_.file_header_.types[i], values[i], index_handler_.file_header_.lens[i])){
      return false;
    }
  }
  return true;
...
bool BplusTreeScanner::satisfy_one_condition(const char *pkey, AttrType attr_type, const char* value, int len) {
  int i1=0,i2=0;
  float f1=0,f2=0;
...
bool BplusTreeScanner::satisfy_condition(const char *pkey) {
  return satisfy_one_condition(pkey, index_handler_.file_header_.attr_type, value_, index_handler_.file_header_.attr_length);
}
